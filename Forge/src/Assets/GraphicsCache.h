...
namespace Forge
{
    ...
    {
        return left.Source == right.Source && left.Type == right.Type && left.Flags == right.Flags &&
...
template<>
struct std::hash<Forge::AssetLocation>
{
public:
    inline size_t operator()(const Forge::AssetLocation& location) const
    {
...
namespace Forge
{
    ...
    public:
        ...
        static Ref<TextureCube> LoadTextureCube(const std::string& front, const std::string& back,
          const std::string& left, const std::string& right, const std::string& bottom, const std::string& top,
          AssetFlags flags = AssetFlags_None);
        ...
        // Meshes
        inline static Ref<Mesh> SquareMesh()
        {
            CreateSquareMesh();
            return s_SquareMesh;
        }
        inline static Ref<Mesh> CubeMesh()
        {
            CreateCubeMesh();
            return s_CubeMesh;
        }
        inline static Ref<Mesh> SphereMesh()
        {
            CreateSphereMesh();
            return s_SphereMesh;
        }
        ...
        static Ref<Material> PbrColorMaterial(
          const Color& albedo = COLOR_WHITE, float roughness = 0.4f, float metallic = 0.0f, float ao = 1.0f);
        ...
        // Shaders
        inline static Ref<Shader> DefaultColorShader()
        {
            CreateDefaultColorShader();
            return s_DefaultColorShader;
        }
        inline static Ref<Shader> DefaultTextureShader()
        {
            CreateDefaultTextureShader();
            return s_DefaultTextureShader;
        }
        inline static Ref<Shader> LitColorShader(bool useShadows)
        {
            CreateLitColorShader();
            return s_LitColorShader[useShadows ? 1 : 0];
        }
        inline static Ref<Shader> LitTextureShader(bool useShadows)
        {
            CreateLitTextureShader();
            return s_LitTextureShader[useShadows ? 1 : 0];
        }
        inline static Ref<Shader> PbrColorShader(bool useShadows)
        {
            CreatePbrColorShader();
            return s_PbrColorShader[useShadows ? 1 : 0];
        }
        inline static Ref<Shader> PbrTextureShader(bool useShadows)
        {
            CreatePbrTextureShader();
            return s_PbrTextureShader[useShadows ? 1 : 0];
        }
        inline static Ref<Shader> AnimatedDefaultColorShader(int maxJoints)
        {
            return CreateDefaultColorAnimatedShader(maxJoints);
        }
        inline static Ref<Shader> AnimatedLitTextureShader(int maxJoints)
        {
            return CreateLitTextureAnimatedShader(maxJoints);
        }

        inline static Ref<Shader> DefaultShadowShader()
        {
            CreateDefaultShadowShader();
            return s_DefaultShadowShader;
        }
        inline static Ref<Shader> DefaultPointShadowShader()
        {
            CreateDefaultPointShadowShader();
            return s_DefaultPointShadowShader;
        }
        inline static Ref<Shader> DefaultPickShader()
        {
            CreateDefaultPickShader();
            return s_DefaultPickShader;
        }

        inline static ShadowFormationShaderSet DefaultShadowShaders()
        {
            return ShadowFormationShaderSet {DefaultPointShadowShader(), DefaultShadowShader()};
        }

        // Textures
        inline static Ref<Texture2D> WhiteTexture()
        {
            CreateWhiteTexture();
    ...
    private:
        ...
        template<typename T>
        static void RegisterNewAsset(
          AssetLocation location, const Ref<T>& asset, std::unordered_map<AssetLocation, std::weak_ptr<T>>& map)
        {
            if constexpr (std::is_same_v<T, Shader>)
