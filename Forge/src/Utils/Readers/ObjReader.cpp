...
namespace Forge
{
	...
	void HandleVertex(const std::string_view& line, std::vector<glm::vec3>& vertices)
	{
		...
		FORGE_ASSERT(firstSpace != std::string_view::npos && secondSpace != std::string_view::npos && thirdSpace != std::string_view::npos, "Invalid vertex line");
		glm::vec3 vertex;
		...
		std::from_chars(x.data(), x.data() + x.size(), vertex.x);
		std::from_chars(y.data(), y.data() + y.size(), vertex.y);
		std::from_chars(z.data(), z.data() + z.size(), vertex.z);
	...
	void HandleVertexNormal(const std::string_view& line, std::vector<glm::vec3>& normals)
	{
		...
		size_t secondSpace = line.find_first_of(' ', firstSpace + 1);
		size_t thirdSpace = line.find_first_of(' ', secondSpace + 1);
		FORGE_ASSERT(firstSpace != std::string_view::npos && secondSpace != std::string_view::npos && thirdSpace != std::string_view::npos, "Invalid normal line");
		glm::vec3 normal;
		std::string_view x = line.substr(firstSpace + 1, secondSpace - firstSpace - 1);
		std::string_view y = line.substr(secondSpace + 1, thirdSpace - secondSpace - 1);
		std::string_view z = line.substr(thirdSpace + 1);
		std::from_chars(x.data(), x.data() + x.size(), normal.x);
		std::from_chars(y.data(), y.data() + y.size(), normal.y);
		std::from_chars(z.data(), z.data() + z.size(), normal.z);
	...
	void HandleTexCoord(const std::string_view& line, std::vector<glm::vec2>& texCoords)
	{
		size_t firstSpace = line.find_first_of(' ');
		size_t secondSpace = line.find_first_of(' ', firstSpace + 1);
		FORGE_ASSERT(firstSpace != std::string_view::npos && secondSpace != std::string_view::npos, "Invalid texcoord line");
		glm::vec2 texCoord;
		std::string_view x = line.substr(firstSpace + 1, secondSpace - firstSpace - 1);
		std::string_view y = line.substr(secondSpace + 1);
		std::from_chars(x.data(), x.data() + x.size(), texCoord.x);
		std::from_chars(y.data(), y.data() + y.size(), texCoord.y);
	...
	FaceIndices ReadFaceBlock(const std::string_view& block)
	{
		...
		FaceIndices result;
		std::from_chars(vertexIndex.data(), vertexIndex.data() + vertexIndex.size(), result.Vertex);
		if (texCoordIndex.empty())
			result.TexCoord = -1;
		else
			std::from_chars(texCoordIndex.data(), texCoordIndex.data() + texCoordIndex.size(), result.TexCoord);
		if (normalIndex.empty())
			result.Normal = -1;
		else
			std::from_chars(normalIndex.data(), normalIndex.data() + normalIndex.size(), result.Normal);
		return result;
	}
	...
	void HandleFace(const std::string_view& line, std::vector<Face>& faces)
	{
		...
		size_t secondSpace = line.find_first_of(' ', firstSpace + 1);
		size_t thirdSpace = line.find_first_of(' ', secondSpace + 1);
		FORGE_ASSERT(firstSpace != std::string_view::npos && secondSpace != std::string_view::npos && thirdSpace != std::string_view::npos, "Invalid face line");
