...
namespace Forge
{
	...
	static void SerializeEntity(YAML::Emitter& emitter, Entity entity)
	{
		...
		if (entity.HasComponent<PointLightComponent>())
		{
			...
			emitter << YAML::Key << "ShadowWidth" << YAML::Value << (light.Shadows.Enabled ? light.Shadows.RenderTarget->GetWidth() : 0);
			emitter << YAML::Key << "ShadowHeight" << YAML::Value << (light.Shadows.Enabled ? light.Shadows.RenderTarget->GetHeight() : 0);
	...
	static void DeserializeEntity(YAML::Node node, Entity entity)
	{
		...
		if (pointLightComponent)
		{
			...
			if (shadowsEnabled)
			{
				light.CreateShadowPass(pointLightComponent["ShadowWidth"].as<float>(), pointLightComponent["ShadowHeight"].as<float>());
			}
		}
		...
		if (directionalLightComponent)
		{
			...
			if (shadowsEnabled)
			{
				light.CreateShadowPass(directionalLightComponent["ShadowWidth"].as<float>(), directionalLightComponent["ShadowHeight"].as<float>());
			}
		}
		...
		if (modelRendererComponent)
		{
			...
			for (YAML::Node submodelNode : modelNode)
			{
				...
				shaders.PickShader = GraphicsCache::GetAsset<Shader>(submodelNode["Material"]["Shaders"][int(RenderPass::Pick)].as<AssetLocation>());
				shaders.WithoutShadowShader = GraphicsCache::GetAsset<Shader>(submodelNode["Material"]["Shaders"][int(RenderPass::WithoutShadow)].as<AssetLocation>());
				shaders.WithShadowShader = GraphicsCache::GetAsset<Shader>(submodelNode["Material"]["Shaders"][int(RenderPass::WithShadow)].as<AssetLocation>());
				shaders.ShadowFormationShaders.PointShadow = GraphicsCache::GetAsset<Shader>(submodelNode["Material"]["Shaders"][int(RenderPass::PointShadowFormation)].as<AssetLocation>());
				shaders.ShadowFormationShaders.Shadow = GraphicsCache::GetAsset<Shader>(submodelNode["Material"]["Shaders"][int(RenderPass::ShadowFormation)].as<AssetLocation>());
				...
				for (const UniformSpecification& specification : uniforms.GetUniforms())
				{
					...
					case ShaderDataType::Int:
						break;
					case ShaderDataType::Float:
						uniforms.SetUniform(specification.VariableName, uniformsNode[specification.VariableName]["Value"].as<float>());
						break;
					case ShaderDataType::Float2:
						uniforms.SetUniform(specification.VariableName, uniformsNode[specification.VariableName]["Value"].as<glm::vec2>());
						break;
					case ShaderDataType::Float3:
						uniforms.SetUniform(specification.VariableName, uniformsNode[specification.VariableName]["Value"].as<glm::vec3>());
						break;
					case ShaderDataType::Float4:
						uniforms.SetUniform(specification.VariableName, uniformsNode[specification.VariableName]["Value"].as<Color>());
						break;
					case ShaderDataType::Sampler1D:
					case ShaderDataType::Sampler2D:
						uniforms.SetUniform(specification.VariableName, GraphicsCache::GetAsset<Texture2D>(uniformsNode[specification.VariableName]["Value"].as<AssetLocation>()));
						break;
					case ShaderDataType::Sampler3D:
					case ShaderDataType::SamplerCube:
						uniforms.SetUniform(specification.VariableName, GraphicsCache::GetAsset<TextureCube>(uniformsNode[specification.VariableName]["Value"].as<AssetLocation>()));
						break;
					}
				}

				submodel.Material->GetSettings().Culling = CullFace(submodelNode["Material"]["Settings"]["CullFace"].as<int>());
				submodel.Material->GetSettings().Mode = PolygonMode(submodelNode["Material"]["Settings"]["PolygonMode"].as<int>());
