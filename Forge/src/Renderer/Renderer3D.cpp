...
namespace Forge
{
    ...
    Renderer3D::Renderer3D()
        : m_CurrentScene(),
          m_CurrentRenderPass(),
          m_RenderImGui(false),
          m_CurrentShadowLightIndex(0),
          m_Context(),
          m_ClearedFramebuffers(),
    ...
    void Renderer3D::BeginScene(
      const Ref<Framebuffer>& framebuffer, const CameraData& camera, const std::vector<LightSource>& lightSources)
    {
        m_CurrentScene = {
          framebuffer,
          camera,
    ...
    {
        // Skip if we have already rendered this light
        if (m_ShadowFramebuffers.find(pass.RenderTarget.get()) != m_ShadowFramebuffers.end())
            return;
        m_ShadowFramebuffers.insert(pass.RenderTarget.get());
        ...
        if (pass.Light->Type == LightType::Point)
        {
            m_CurrentRenderPass = RenderPass::PointShadowFormation;
            glm::mat4 matrices[6];
            GetCameraTransformsFromLightSource(
              pass.Light->Position, pass.RenderTarget->GetAspect(), pass.Light->ShadowFrustum, matrices);
            m_Context.SetShadowPointMatrices(pass.Light->Position, matrices);
        }
        else
        {
            m_CurrentRenderPass = RenderPass::ShadowFormation;
            shadowScene.Camera = CreateCameraFromLightSource(
              pass.Light->Position, pass.Light->Direction, pass.RenderTarget, pass.Light->ShadowFrustum);
            pass.Light->LightSpaceTransform =
              shadowScene.Camera.Frustum.ProjectionMatrix * shadowScene.Camera.ViewMatrix;
        }
    ...
    void Renderer3D::SetupScene(const SceneData& data)
    {
        ...
        if (m_CurrentRenderPass != RenderPass::Pick)
        {
            if (m_CurrentRenderPass == RenderPass::PointShadowFormation ||
                m_CurrentRenderPass == RenderPass::ShadowFormation)
            {
    ...
    {
        if (m_CurrentRenderPass == RenderPass::PointShadowFormation &&
            !data.Options.ShadowMask.test(m_CurrentShadowLightIndex))
            return;
        ...
        for (const Model::SubModel& submodel : model->GetSubModels())
        {
            ...
            if (m_CurrentRenderPass == RenderPass::PointShadowFormation ||
                m_CurrentRenderPass == RenderPass::ShadowFormation)
            {
                // Skip model if it does not cast shadows
    ...
    CameraData Renderer3D::CreateCameraFromLightSource(const glm::vec3& lightPosition, const glm::vec3& lightDirection,
      const Ref<Framebuffer>& renderTarget, const Frustum& frustum) const
    {
        CameraData camera;
    ...
    void Renderer3D::GetCameraTransformsFromLightSource(
      const glm::vec3& lightPosition, float aspect, const Frustum& frustum, glm::mat4 transforms[6])
    {
        glm::mat4 projection = glm::perspective(glm::radians(90.0f), aspect, frustum.NearPlane, frustum.FarPlane);
        transforms[0] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {1.0, 0.0, 0.0}, glm::vec3 {0, -1, 0});
        transforms[1] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {-1.0, 0.0, 0.0}, glm::vec3 {0, -1, 0});
        transforms[2] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {0.0, 1.0, 0.0}, glm::vec3 {0, 0, 1});
        transforms[3] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {0.0, -1.0, 0.0}, glm::vec3 {0, 0, -1});
        transforms[4] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {0.0, 0.0, 1.0}, glm::vec3 {0, -1, 0});
        transforms[5] =
          projection * glm::lookAt(lightPosition, lightPosition + glm::vec3 {0.0, 0.0, -1.0}, glm::vec3 {0, -1, 0});
    }
